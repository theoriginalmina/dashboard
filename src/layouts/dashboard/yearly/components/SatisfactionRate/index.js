...
const SatisfactionRate = () => {
  ...
  const goal = 100; // Define your sales goal here

  const { user } = useAuth();
  const userId = user?.id;

  const getYearlyCarsSold = async () => {
    ...
    const { data, error } = await supabase
      ...
      .select('count, date')
      .eq('user_id', userId)
      .gte('date', `${currentYear}-01-01`) // Filter by the first day of the current year and onwards
  ...
  // Use the getYearlyCarsSold function to fetch the total cars sold for the current year
  const [totalCarsSold, setTotalCarsSold] = useState(0);

  useEffect(() => {
    getYearlyCarsSold().then((count) => {
    ...
    <Card sx={{ height: '340px' }}>
      <VuiBox display='flex' flexDirection='column'>
        ...
        <VuiTypography color='text' variant='caption' display='inline-block' fontWeight='regular'>
          {totalCarsSold} Units Sold This Year
        </VuiTypography>
